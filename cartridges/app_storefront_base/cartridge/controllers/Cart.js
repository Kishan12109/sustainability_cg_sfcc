...
server.post('AddProduct', function (req, res, next) {
    ...
    var number;
    var sustainaibleProduct = false;
    if (currentBasket) {
        var ProductLineItems = currentBasket.allProductLineItems;
        collections.forEach(ProductLineItems, function (pli) {
            if (pli.UUID === result.uuid) {
                if (pli.product.custom.productBadge.value === 'sustainability') {
                    sustainaibleProduct = true;
                } else {
                    sustainaibleProduct = false;
                }
                if (pli.product.custom.productTotalCarbonPoints) {
                    number = pli.product.custom.productTotalCarbonPoints;
                } else {
                    number = 0;
                }
                Transaction.wrap(function () {
                    pli.custom.carbonPoints = number; // eslint-disable-line no-param-reassign
                    pli.custom.isSustainaibleProduct = sustainaibleProduct; // eslint-disable-line no-param-reassign
                });
            }
        });
    }
...
server.get(
    ...
    function (req, res, next) {
        ...
        var enableNeutralizeOptionsCart = Site.getCurrent().getCustomPreferenceValue('enableNeutralizeOptionsCart');
        viewData.enableSustainability = enableSustainability;
        viewData.enableNeutralizeOptionsCart = enableNeutralizeOptionsCart;
        var Customer = require('dw/customer/Customer');
        var currentCustomer = req.currentCustomer;
        if (!empty(currentCustomer)) {
           viewData.registeredUser = currentCustomer instanceof Customer ? (currentCustomer.authenticated && currentCustomer.registered) : (currentCustomer.raw.authenticated && currentCustomer.raw.registered);
        } else {
        ...
        res.setViewData({ reportingURLs: reportingURLs, treeCostValues: treeCostValues });
...
server.post(
    ...
    function (req, res, next) {
        ...
        var CartModel = require('*/cartridge/models/cart');
        var currentBasket = BasketMgr.getCurrentBasket();
        var singleOptionCost = 0;
        var neutralizeConfigs = Site.getCurrent().getCustomPreferenceValue('addTree') || "[]";
        
        neutralizeConfigs = JSON.parse(neutralizeConfigs);
        var mappingPointsValue = new HashMap();
        for (var index = 0; index < neutralizeConfigs.length; index++) {
            mappingPointsValue = put(neutralizeConfigs[index].id, neutralizeConfigs[index].points);
            singleOptionCost = neutralizeConfigs[index].value;
        }
        var treeCostId = req.querystring.id;
        if (currentBasket) {
            var shipmentLineItems = currentBasket.shipments;
            if (singleOptionCost !== 0) {
                collections.forEach(shipmentLineItems, function (ship) {
                    Transaction.wrap(function () {
                        var sustainShippingLineItem = ship.createShippingLineItem(treeCostId);
                        sustainShippingLineItem.setPriceValue(singleOptionCost);
                        sustainShippingLineItem.setLineItemText('tree planting cost');
