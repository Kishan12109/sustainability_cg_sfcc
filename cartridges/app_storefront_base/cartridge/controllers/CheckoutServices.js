...
server.post('PlaceOrder', server.middleware.https, function (req, res, next) {
    ...
    var totalCarbonPoints = COHelpers.totalCarbonPoints(order);
        var profile = session.customer.profile;
        var shipLineItems = order.shipments[0].shippingLineItems;
        Transaction.wrap(function () {
            var totalCarbonNeutralizedPoints = 0;
            for (var index = 0; index < shipLineItems.length; index++) {
                if (!empty(shipLineItems[index].custom) && !empty(shipLineItems[index].custom.neutralizedPoints)) {
                    totalCarbonNeutralizedPoints += shipLineItems[index].custom.neutralizedPoints;
                    order.custom.totalCarbonNeutralizedPoints = totalCarbonNeutralizedPoints;
                }
            }
            if (totalCarbonPoints) {
                order.custom.totalCarbonPoints = Number(totalCarbonPoints);
            }

          
            if (profile) {
                if (empty(profile.custom.carbonPointsNeutralized)) {
                    profile.custom.carbonPointsNeutralized = 0;
                }
                profile.custom.carbonPointsNeutralized += order.custom.totalCarbonNeutralizedPoints;

                if (empty(profile.custom.carbonPointsConsumed)) {
                    profile.custom.carbonPointsConsumed = 0;
                }
                profile.custom.carbonPointsConsumed += order.custom.totalCarbonPoints;
            }
        });
