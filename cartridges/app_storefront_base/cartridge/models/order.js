...
function getTotalCarbonPoints(productLineItemsModel) {
    var totalCarbonPoints = 0;
    // eslint-disable-next-line linebreak-style
    productLineItemsModel.items.forEach(function (item) {
        var itemCarbonPoints = 0;
        if (item.quantity > 0) {
            itemCarbonPoints = item.quantity * item.carbonPoints;
        // eslint-disable-next-line linebreak-style
        } 
        totalCarbonPoints += itemCarbonPoints;
    });
    return totalCarbonPoints;
}
...
function OrderModel(lineItemContainer, options) {
    ...
    if (!lineItemContainer) {
        ...
        this.totalCarbonPoints = getTotalCarbonPoints(productLineItemsModel);
        ...
        if (lineItemContainer.custom && 'totalCarbonNeutralizedPoints' in lineItemContainer.custom && !empty(lineItemContainer.custom.totalCarbonNeutralizedPoints) && lineItemContainer.custom.totalCarbonNeutralizedPoints !== null) {
            this.totalCarbonNeutralizedPoints = lineItemContainer.custom.totalCarbonNeutralizedPoints;
        }
