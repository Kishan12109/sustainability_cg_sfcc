...
function getTreeShipCost(lineItemContainer) {
    var treeCost = 0;
    var treeShipCost = [];
    var shipments = lineItemContainer.shipments;
    collections.forEach(shipments, function (item) {
        for (var i = 0; i < item.shippingLineItems.length; i++) {
            if (item.shippingLineItems[i].custom.isSustainable === true) {
                treeCost += item.shippingLineItems[i].basePrice.value;
            }
        }
    });
    var defaultCurrency = Site.current.defaultCurrency;
    var originalShippingCost = (lineItemContainer.shippingTotalPrice.value - treeCost);
    var formatTreeCost = formatMoney(new Money(treeCost, defaultCurrency));
    var formatShipCost = formatMoney(new Money(originalShippingCost, defaultCurrency));
    treeShipCost.push(formatTreeCost, formatShipCost);
    return treeShipCost;
}

// eslint-disable-next-line require-jsdoc
function getAllTreeCostIdFlags(lineItemContainer) {
    var treeCostOne = false;
    var treeCostTwo = false;
    var treeCostThree = false;
    var treeCostFour = false;
    var allTreeCostIdFlags = [];
    var shipments = lineItemContainer.shipments;
    collections.forEach(shipments, function (item) {
        for (var i = 0; i < item.shippingLineItems.length; i++) {
            if (item.shippingLineItems[i].custom.isSustainable === true && item.shippingLineItems[i].ID === 'treeCost1') {
                treeCostOne = true;
            }
            if (item.shippingLineItems[i].custom.isSustainable === true && item.shippingLineItems[i].ID === 'treeCost2') {
                treeCostTwo = true;
            }
            if (item.shippingLineItems[i].custom.isSustainable === true && item.shippingLineItems[i].ID === 'treeCost3') {
                treeCostThree = true;
            }
            if (item.shippingLineItems[i].custom.isSustainable === true && item.shippingLineItems[i].ID === 'treeCost4') {
                treeCostFour = true;
            }
        }
    });
    allTreeCostIdFlags.push(treeCostOne, treeCostTwo, treeCostThree, treeCostFour);
    return allTreeCostIdFlags;
}
...
function totals(lineItemContainer) {
    if (lineItemContainer) {
        ...
        var treeShipCost = getTreeShipCost(lineItemContainer);
        this.totalTreeCost = treeShipCost[0];
        this.totalShippingCost = treeShipCost[1];
        var treeCostIdFlags = getAllTreeCostIdFlags(lineItemContainer);
        this.treeCostOne = treeCostIdFlags[0];
        this.treeCostTwo = treeCostIdFlags[1];
        this.treeCostThree = treeCostIdFlags[2];
        this.treeCostFour = treeCostIdFlags[3];
