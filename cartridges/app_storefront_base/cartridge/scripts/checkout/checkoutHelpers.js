...
function totalCarbonPoints(lineItemCtnr) {
    var carbonPoints = 0;
    var productLineItems = lineItemCtnr.allProductLineItems;
            // eslint-disable-next-line linebreak-style
    collections.forEach(productLineItems, function (item) {
        var itemCarbonPoints = 0;
        if (item.quantity > 0 && item.custom && 'carbonPoints' in item.custom && !empty(item.custom.carbonPoints))  {
            itemCarbonPoints = item.quantity * item.custom.carbonPoints;
                // eslint-disable-next-line linebreak-style
    } 
        carbonPoints += itemCarbonPoints;
    });
    return carbonPoints;
}
